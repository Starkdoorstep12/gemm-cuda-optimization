...
__global__ void sgemm_naive(int M, int N, int K, float alpha,
                             ...
                             float beta, float *C) {
    // NAIVE mapping (deliberately bad for coalescing, per Boehm's kernel 1):
    // row varies with threadIdx.x -> consecutive threads in a warp hit
    // different, strided rows of A and strided elements of C.
