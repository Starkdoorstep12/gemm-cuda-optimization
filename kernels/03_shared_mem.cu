...
void run_sgemm_shared_mem(int M, int N, int K, float alpha,
                           const float *A, const float *B,
                           float beta, float *C) {
...
int main(int argc, char **argv) {
    ...
    if (M % BLOCKSIZE != 0 || N % BLOCKSIZE != 0 || K % BLOCKSIZE != 0) {
        fprintf(stderr, "WARNING: M, N, K should be multiples of %d for this kernel "
                         "(no boundary checks, matching Boehm's version).\n", BLOCKSIZE);
    }

    printf("Shared-Mem-Tiled GEMM: M=%d N=%d K=%d\n", M, N, K);
